...
def load_feature_ranking(veto_path: str = "data/veto_average_results (1).csv") -> pd.DataFrame:
    """Load and cache feature importance ranking"""
    global FEATURE_RANKING
    if FEATURE_RANKING is None:
        # Try different paths (for running from root or from src/api)
        possible_paths = [veto_path, f"../../{veto_path}", "veto_average_results (1).csv"]
        veto_file = None
        ...
        if veto_file is None:
            raise FileNotFoundError(f"Feature ranking file not found. Tried: {possible_paths}")
        
        FEATURE_RANKING = pd.read_csv(veto_file)
